...
<!--      </el-popconfirm>-->
      <!-- <el-upload action="http://localhost:9090/movie/import" :show-file-list="false" accept="xlsx" :on-success="handleExcelImportSuccess" style="display: inline-block">
...
<!--      <el-table-column prop="mid" label="电影id"></el-table-column>-->
      <el-table-column prop="score" label="平均评分"></el-table-column>
      ...
      <el-table-column label="封面"><template slot-scope="scope"><el-image style="width: 100px; height: 100px" :src="scope.row.img" :preview-src-list="[scope.row.img]"></el-image></template></el-table-column>
...
<script>
  export default {
    ...
    methods: {
      ...
      save() {
        this.request.post("/movie", this.form).then(res => {
          if (res.code === '200') {
      ...
      del(id) {
        this.request.delete("/movie/" + id).then(res => {
          if (res.code === '200') {
      ...
      delBatch() {
        ...
        this.request.post("/movie/del/batch", ids).then(res => {
          if (res.code === '200') {
      ...
      exp() {
        window.open("http://localhost:9090/movie/export")
      },
